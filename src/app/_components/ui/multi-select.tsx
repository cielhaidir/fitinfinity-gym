   ...
   return (
      <PopoverPrimitive.Trigger ref={forwardedRef as any} asChild>
         <div
            ...
            {...props}
            className={cn(
               "border-neutral-200 ring-offset-white focus:ring-neutral-950 flex size-full min-h-10 items-center justify-between whitespace-nowrap rounded-md border bg-transparent px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-1 [&>span]:line-clamp-1 dark:border-neutral-800 dark:ring-offset-neutral-950 dark:focus:ring-neutral-300",
...
>(
   (
      ...
      if (!value.length || !firstRendered) {
         return (
            <span className="text-neutral-500 pointer-events-none dark:text-neutral-400">
               {placeholder}
            </span>
         )
      }
            ...
            >
               {renderItems.map((value) => {
                  ...
                  const el = (
                     ...
                     >
                        <span>{child}</span>
                        <X className="text-neutral-500 group-hover/multi-select-badge:text-foreground ml-1 size-3 dark:text-neutral-400" />
                     </Badge>
                  )
               ...
               {renderRemain ? (
                  <span className="text-neutral-500 py-.5 text-xs leading-4 dark:text-neutral-400">
                     +{renderRemain}
                  </span>
               ) : null}
            </div>
   ...
   (
      ...
      return (
         <CommandItem
            ...
            value={value}
            className={cn(
               disabled && "text-neutral-500 cursor-not-allowed dark:text-neutral-400",
               className,
            )}
            disabled={disabled}
