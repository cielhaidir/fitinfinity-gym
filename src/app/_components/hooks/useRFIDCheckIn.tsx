...
export function RFIDProvider({ children }: RFIDProviderProps) {
  ...
  const { data: foundMember, isLoading: isSearching } = api.member.findByRFID.useQuery(
  ...
  useEffect(() => {
    if (currentRFID && !isSearching) {
      if (foundMember) {
        console.log("Member found, opening modal:", foundMember);
        setSelectedMemberForCheckIn(foundMember);
        setIsCheckInModalOpen(true);
        toast.success(`Member found: ${foundMember.user.name}`);
      } else {
  ...
  }, [currentRFID, foundMember, isSearching]);
  ...
  const processRFIDBuffer = useCallback(() => {
    ...
    // Check if buffer contains exactly 10 digits
    if (trimmedBuffer.length === 10 && /^\d{10}$/.test(trimmedBuffer)) {
      console.log("Valid RFID detected:", trimmedBuffer);
      handleRFIDScan(trimmedBuffer);
    } else {
      console.log("Invalid RFID format:", trimmedBuffer);
    }
    
    // Clear buffer after processing
    setRfidBuffer("");
  }, [rfidBuffer, handleRFIDScan]);
  ...
  const handleKeyDown = useCallback((event: KeyboardEvent) => {
    ...
    if (event.key === "Enter") {
      ...
      setRfidBuffer(prev => {
        ...
        console.log("Adding digit:", event.key, "New buffer:", newBuffer);
        // Limit buffer to 10 digits to prevent overflow
        return newBuffer.length > 10 ? newBuffer.slice(-10) : newBuffer;
      });
