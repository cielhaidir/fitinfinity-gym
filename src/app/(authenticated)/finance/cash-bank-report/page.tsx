...
const CashBankReportPage = () => {
  ...
  // Query for getting all data for Excel export (without pagination)
  const { refetch: refetchExportData } = api.cashBankReport.getCashBankReportForExport.useQuery({
    startDate,
    endDate,
    balanceAccountId: undefined, // Get all accounts for export
    sortBy,
    sortOrder,
  }, {
    enabled: false, // Only fetch when needed for export
  });
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const allData = await refetchExportData();
      const exportItems = allData.data?.items || [];
      ...
      // Group items by balance account
      const groupedByAccount = exportItems.reduce((acc, item) => {
        const accountName = item.balanceAccount || 'No Account';
        if (!acc[accountName]) {
          acc[accountName] = [];
        }
        acc[accountName].push(item);
        return acc;
      }, {} as Record<string, CashBankReportItem[]>);

      const wb = XLSX.utils.book_new();

      // Create a sheet for each balance account
      Object.entries(groupedByAccount).forEach(([accountName, accountItems]) => {
        const dataToExport = accountItems.map(item => ({
        ...
        const totalDebits = accountItems.reduce((sum, item) => sum + item.debit, 0);
        const totalCredits = accountItems.reduce((sum, item) => sum + item.credit, 0);
        const netBalance = totalCredits - totalDebits;

        // Add summary rows at the top
        const summaryData = [
          { 'Reference ID': 'SUMMARY', 'Date': '', 'Type': '', 'Description': '', 'Debit OC': '', 'Credit OC': '', 'Balance Account': '' },
          { 'Reference ID': 'Total Credits', 'Date': '', 'Type': '', 'Description': '', 'Debit OC': '', 'Credit OC': totalCredits, 'Balance Account': '' },
          { 'Reference ID': 'Total Debits', 'Date': '', 'Type': '', 'Description': '', 'Debit OC': totalDebits, 'Credit OC': '', 'Balance Account': '' },
          { 'Reference ID': 'Net Balance', 'Date': '', 'Type': '', 'Description': '', 'Debit OC': '', 'Credit OC': netBalance, 'Balance Account': '' },
          { 'Reference ID': '', 'Date': '', 'Type': '', 'Description': '', 'Debit OC': '', 'Credit OC': '', 'Balance Account': '' }, // Empty row
          { 'Reference ID': 'DETAILS', 'Date': '', 'Type': '', 'Description': '', 'Debit OC': '', 'Credit OC': '', 'Balance Account': '' },
        ];

        const combinedData = [...summaryData, ...dataToExport];
        ...
        ws['!cols'] = [
          ...
          { width: 18 }, // Debit OC
          { width: 18 }, // Credit OC
          { width: 20 }, // Balance Account
        ];

        // Clean sheet name (Excel doesn't allow certain characters)
        const cleanSheetName = accountName
          .replace(/[\\\/\?\*\[\]]/g, '_')
      ...
      // If no specific account is selected, also create a summary sheet
      if (!balanceAccountId) {
        const summaryByAccount = Object.entries(groupedByAccount).map(([accountName, accountItems]) => {
          const totalDebits = accountItems.reduce((sum, item) => sum + item.debit, 0);
          const totalCredits = accountItems.reduce((sum, item) => sum + item.credit, 0);
          const netBalance = totalCredits - totalDebits;
          ...
          return {
            ...
            'Transaction Count': accountItems.length,
          };
        });

        const summaryWs = XLSX.utils.json_to_sheet(summaryByAccount);
        summaryWs['!cols'] = [
          { width: 25 }, // Balance Account
          { width: 18 }, // Total Credits
          { width: 18 }, // Total Debits
          { width: 18 }, // Net Balance
        ...
        wb.Sheets = { Summary: summaryWs, ...wb.Sheets };
      }

      // Generate filename with date range
      const startDateStr = startDate.toLocaleDateString('id-ID').replace(/\//g, '-');
      const endDateStr = endDate.toLocaleDateString('id-ID').replace(/\//g, '-');
      const filename = `Cash_Bank_Report_${startDateStr}_to_${endDateStr}.xlsx`;
    ...
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b border-gray-200 dark:border-gray-700 mb-6 gap-4">
        ...
        <button
          ...
          className="inline-flex items-center px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isExporting ? (
            <>
