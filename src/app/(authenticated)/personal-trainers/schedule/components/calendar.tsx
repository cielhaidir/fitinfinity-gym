                      ...
                      {days.map((day) => {
                        ...
                        return (
                          <DroppableCell
                            key={`${dateStr}-${hour}`}
                            id={`${dateStr}-${hour}`}
                            className={`h-[64px] cursor-pointer border-r border-border p-1 align-top hover:bg-muted ${isToday(day) ? "bg-muted/50" : ""} `}
                      ...
                      <div className="mt-1 max-h-[80px] space-y-1 overflow-y-auto">
                        {sessions.slice(0, 3).map((session) => (
                          <div
                            ...
                            } cursor-pointer rounded text-xs text-foreground transition-colors`}
                            onClick={(e) => handleSessionClick(e, session)}
                          >
                            ...
                            {session.status === "CANCELED" && (
                              <div className="mt-1 text-xs font-bold text-destructive-foreground">
                                Dibatalkan
                              </div>
                            )}
                            {session.status === "ONGOING" && (
                              <div className="mt-1 text-xs font-bold text-yellow-900">
                                Sedang Berlangsung
                              </div>
                            )}
                            {session.status === "ENDED" &&
