...
export default function AdminPaymentValidationPage() {
    ...
    const { 
        ...
        error: queryError,
        refetch 
    } = api.paymentValidation.listWaiting.useQuery(
        { page, limit }
    );
    ...
    const acceptMutation = api.paymentValidation.accept.useMutation({
        onSuccess: (data) => {
            toast.success(`Payment accepted. Subscription ID: ${data.subscriptionId}`);
    ...
    const handleConfirm = async () => {
        if (selectedAction === 'accept' && selectedValidationId && selectedBalanceId) {
            await acceptMutation.mutateAsync({ id: selectedValidationId, balanceId: selectedBalanceId });
        ...
        return (
            <div className="container mx-auto p-5">
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    ...
                    <p className="text-red-600 mt-1">{queryError.message}</p>
                    <Button 
                        variant="outline" 
                        className="mt-4"
    ...
    return (
        <div className="container mx-auto p-5 md:p-8">
            <div className="flex items-center justify-between space-y-2 mb-6">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Payment Validations</h2>
                            ...
                            <Link href={selectedImageUrl} target="_blank" rel="noopener noreferrer">
                                <Image 
                                    ...
                                    onError={(e) => { 
                                        ...
                                        (e.target as HTMLImageElement).src = '/placeholder-error.png'; // Fallback image
            ...
            <Dialog open={showConfirmModal} onOpenChange={setShowConfirmModal}>
                <DialogContent>
                    ...
                    {selectedAction === 'accept' && (
                        <div className="mb-4">
                            <label htmlFor="balance-account" className="block mb-2 font-medium">
                            ...
                            {accounts.length > 0 ? (
                                ...
                                <div className="text-sm text-muted-foreground">No balance account available.</div>
                            )}
                        </div>
                    )}
                    <DialogFooter>
                        <Button onClick={handleConfirm} disabled={selectedAction === 'accept' && !selectedBalanceId}>
                            Confirm
                        </Button>
