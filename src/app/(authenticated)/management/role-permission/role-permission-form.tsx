...
export const RolePermissionForm: React.FC<RolePermissionFormProps> = ({
  ...
  const allFilteredPermissionIds = useMemo(() => {
    return filteredPermissions.map(permission => permission.id);
  }, [filteredPermissions]);

  const allSelected = useMemo(() => {
    return allFilteredPermissionIds.every(id => selectedPermissionIds.includes(id));
  }, [allFilteredPermissionIds, selectedPermissionIds]);

  const handleSelectAll = () => {
    if (allSelected) {
      const remainingPermissions = selectedPermissionIds.filter(id => !allFilteredPermissionIds.includes(id));
      onPermissionsChange(remainingPermissions);
    } else {
      const combinedPermissions = [...new Set([...selectedPermissionIds, ...allFilteredPermissionIds])];
      onPermissionsChange(combinedPermissions);
    }
  };  
