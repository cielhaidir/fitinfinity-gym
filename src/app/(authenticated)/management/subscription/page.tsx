...
export default function PaymentHistoryPage() {
    ...
    const { data: transactions, isLoading } = api.paymentValidation.getAllPayments.useQuery(
        { page, limit },
        {
            enabled: !!session,
    ...
    const goToPaymentStatus = (orderReference: string, memberId: string) => {
        console.log("Navigating to payment status for order:", orderReference, "and member:", memberId);
        if (memberId) {
            router.push(`/checkout/confirmation/${memberId}?orderRef=${orderReference}`);
        }
    };
