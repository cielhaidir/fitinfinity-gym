...
export default function SubscriptionPage({
  ...
  const {
    ...
    error: memberError,
  } = api.member.detail.useQuery({ id: memberID });
  const {
    data: subs = { items: [], total: 0, page: 1, limit: 10 },
    isLoading,
    error,
    refetch,
  } = api.subs.getByIdMember.useQuery({ memberId: memberID, page, limit });
      ...
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="mt-5 text-2xl font-bold">
            {member?.user?.name}'s Subscriptions
          </h2>
