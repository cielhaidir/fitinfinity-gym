...
export default function EditPurchaseOrderPage() {
  ...
  // Queries
  const { data: order, isLoading: orderLoading, error } = api.purchaseOrder.getById.useQuery(
    { id: orderId },
    { enabled: !!orderId }
  );
  ...
  useEffect(() => {
    if (order && !isInitialized) {
      ...
      setExpectedDate(
        order.expectedDate
          ? format(new Date(order.expectedDate), "yyyy-MM-dd")
          : ""
      );
      ...
      <ProtectedRoute>
        <div className="flex flex-col items-center justify-center py-12">
          <p className="text-destructive">
            This purchase order cannot be edited (Status: {order.status})
          </p>
          <Button
            variant="outline"
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Manage items in the purchase order
                    </CardDescription>
                  </div>
